...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get("/api/v1/search", async (request, reply) => {
    ...
    const category = String((request.query as Record<string, string | undefined>).category ?? "").trim().toLowerCase();
    if (!query && !category) {
      return reply.badRequest("q is required");
    }
    const guideResults = query ? searchVeloraGuide(query) : [];
    const documents = await repository.searchDocuments(query, category);
    return { query, category, results: [...guideResults, ...documents].slice(0, 35) };
  });

  app.get("/api/v1/search/categories", async () => {
    return { categories: await repository.listSearchCategories() };
  });
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}body{margin:0;min-height:100vh;background:radial-gradient(circle at 16% 0,rgba(92,200,255,.24),transparent 34%),linear-gradient(180deg,var(--bg),#03101a 80%);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    ...
    .category-bar{display:flex;gap:8px;overflow:auto;padding:12px 0 14px;scrollbar-width:thin}.category-bar button{width:auto;white-space:nowrap;border-radius:999px;padding:9px 12px;background:#071827}.category-bar button.active{background:linear-gradient(135deg,var(--gold),#f6df95);border-color:transparent;color:#07131f}.category-bar span{opacity:.7;margin-left:4px}.search-item{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:14px;align-items:center}.result-actions{display:grid;gap:8px;justify-items:end}.result-actions span{border:1px solid var(--line);border-radius:999px;padding:6px 9px;color:var(--gold);font-size:12px;text-transform:uppercase}
    ...
    .app{grid-template-columns:212px minmax(0,1fr)}.sidebar{padding:14px 10px}.brand{gap:10px}.brand b{letter-spacing:.18em}.brand span{font-size:11px}.logo{width:38px;height:38px;border-radius:13px}.nav{gap:7px}.nav button{min-height:38px;padding:8px 10px;border-radius:11px}.toolbar{height:58px;padding:8px 14px;grid-template-columns:minmax(280px,1fr) auto auto auto auto}.toolbar button,.toolbar .lang-select{min-height:42px}.content{max-width:none;padding:12px 14px 24px}.session-line{margin:6px 0 10px}.dashboard-grid{grid-template-columns:minmax(0,1fr) 290px;gap:12px}.dashboard-hero{min-height:205px;padding:22px;grid-template-columns:minmax(0,1fr) 210px}.dashboard-hero h1{font-size:clamp(34px,3.8vw,48px);max-width:640px}.dashboard-hero p{max-width:610px}.hero-actions{display:flex;gap:10px;flex-wrap:wrap}.hero-actions button{width:auto}.hero-core{width:170px;height:170px}.hero-core span{width:96px;height:96px;border-radius:27px;font-size:54px}.activity-panel .item{padding:10px 12px}.metrics-row{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.metrics-row .card{min-height:70px}.quick-panel{grid-column:1/2}.quick-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.network-panel{grid-column:2}.auth-panel{grid-column:1/-1}.page-head{min-height:82px;padding:12px 16px}.page-head h1{font-size:28px}.search-shell{grid-template-columns:minmax(0,1fr) 280px}.browser-shell{grid-template-columns:280px minmax(0,1fr)}.browser-view iframe{height:calc(100vh - 156px);min-height:390px}.browser-side{position:sticky;top:72px}.category-bar button{width:auto}.split{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}.item button,.panel button{width:auto}@media(max-width:1120px){.app{grid-template-columns:196px minmax(0,1fr)}.dashboard-grid{grid-template-columns:1fr}.network-panel,.quick-panel,.auth-panel{grid-column:1}.activity-panel{grid-column:1}.search-shell,.browser-shell{grid-template-columns:1fr}.browser-side{position:relative;top:auto}}@media(max-width:980px){html,body{width:100%;overflow-x:hidden}.app{display:block}.sidebar{display:none}.main{width:100%;overflow:visible}.toolbar{height:auto!important;min-height:58px;grid-template-columns:minmax(0,1fr) 94px!important;padding:8px 10px}.toolbar .lang-select,.toolbar button:nth-of-type(2),.toolbar button:nth-of-type(3),#profileButton{display:none!important}.toolbar button:first-of-type{display:inline-flex!important;align-items:center;justify-content:center;width:auto!important;margin:0!important}.content{padding:10px 10px calc(92px + env(safe-area-inset-bottom));overflow:visible}.session-line{display:none}.dashboard-grid{grid-template-columns:1fr;gap:10px}.dashboard-hero{padding:17px;grid-template-columns:1fr}.dashboard-hero h1{font-size:32px}.dashboard-hero .hero-actions button{width:auto!important}.hero-core{height:96px;width:100%;border-radius:22px}.hero-core span{width:72px;height:72px;font-size:40px}.metrics-row{grid-template-columns:repeat(2,minmax(0,1fr))}.quick-grid{grid-template-columns:1fr}.page-head{min-height:auto;margin:0 0 10px;padding:14px}.page-head h1{font-size:31px;line-height:1.04}.page-head button{width:auto!important;justify-self:start;margin-top:6px}.search-row{grid-template-columns:1fr}.search-row button{width:100%!important}.category-bar{display:flex!important;flex-wrap:nowrap!important;overflow-x:auto!important;gap:8px;padding:10px 0 12px;scrollbar-width:none}.category-bar::-webkit-scrollbar{display:none}.category-bar button{width:auto!important;min-width:max-content;margin:0!important;padding:10px 14px}.search-shell,.browser-shell,.split{grid-template-columns:1fr!important}.preview-panel{display:none}.browser-side{position:relative;top:auto}.browser-side .category-bar{display:grid!important;grid-template-columns:1fr;overflow:visible!important}.browser-side .category-bar button{width:100%!important;min-width:0}.browser-view{padding:8px}.browser-view iframe{height:58vh!important;min-height:360px}.panel button,.item button{width:auto!important}.tool-output,pre{max-width:100%;overflow:auto}.mobile-tabs{display:grid}}
  </style>
</head>
<body>
  <div class="app">
  ...
  <script>
    ...
    function setDynamicText(id,text){const el=document.getElementById(id);if(!el)return;delete el.dataset.vlSrc;el.textContent=text}
    function setSessionState(){const u=sessionUser();const logged=Boolean(token());const name=u.username||'utente Velora';const mail=(u.mail||u.username||'account')+'';setDynamicText('sessionState',logged?'Connesso come '+name:'Accesso non effettuato');setDynamicText('profileButton',logged?'Profilo':'Accedi');const authPanel=document.getElementById('authPanel');if(authPanel)authPanel.classList.toggle('hidden',logged);const authForm=document.getElementById('authForm');if(authForm)authForm.classList.toggle('hidden',logged);const accountBox=document.getElementById('accountBox');if(accountBox)accountBox.classList.toggle('hidden',!logged);setDynamicText('accountName',name);setDynamicText('accountMail',mail);setDynamicText('profileName',logged?name:'Accesso non effettuato');setDynamicText('profileMail',logged?mail:'Accedi o crea un account dalla Home');setDynamicText('profileStatus',logged?'Sessione attiva':'Non connesso');const actions=document.getElementById('profileActions');if(actions)actions.innerHTML=logged?'<button class="primary" onclick="showModule(\\'mail\\')">Apri VeloMail</button><button onclick="showModule(\\'cloud\\')">Apri Cloud</button><button onclick="logout()">Esci</button>':'<button class="primary" onclick="showModule(\\'home\\')">Vai al login</button>';}
    ...
    async function logout(){const currentToken=token();try{if(currentToken)await api('/api/v1/auth/logout',{method:'POST',headers:{Authorization:'Bearer '+currentToken}})}catch{}clearVeloraSession();clearAuthFields();if(document.getElementById('authMsg'))authMsg.textContent='Sessione chiusa';setSessionState();showModule('home')}
    ...
    async function loadSearchCategories(){try{const data=await api('/api/v1/search/categories');const top=(data.categories||[]).filter(c=>c.zone_count>0).slice(0,24);searchCategoryBar.innerHTML='<button class="'+(!selectedSearchCategory?'active':'')+'" onclick="setSearchCategory(\\'\\')">Tutte</button>'+top.map(c=>'<button class="'+(selectedSearchCategory===c.code?'active':'')+'" onclick="setSearchCategory(\\''+esc(c.code)+'\\')">'+esc(c.code)+' <span>'+esc(c.zone_count)+'</span></button>').join('')}catch(e){searchCategoryBar.innerHTML=''}}
    function setSearchCategory(category){selectedSearchCategory=category;loadSearch()}
    async function loadSearch(){try{await loadSearchCategories();const raw=(currentSection==='oceano'?oceanoQuery.value:searchQuery.value)||globalSearch.value||'';const q=raw||(!selectedSearchCategory?'velora':'');const path='/api/v1/search?q='+encodeURIComponent(q)+'&category='+encodeURIComponent(selectedSearchCategory);const data=await api(path);if(!(data.results||[]).length)track('SEARCH_EMPTY','SEARCH',q||selectedSearchCategory);const html=(data.results||[]).map(r=>'<div class="item search-item"><div><b>'+esc(r.title||r.address)+'</b><p>'+esc(r.description||r.summary||'')+'</p><span class="mono">'+esc(r.address||r.zone||'')+'</span></div><div class="result-actions"><span>'+esc(r.category||'zone')+'</span><button onclick="openZone(\\''+esc(r.address||r.zone||'')+'\\')">Apri</button></div></div>').join('')||item('Search','Nessun risultato disponibile');searchResults.innerHTML=html;if(document.getElementById('oceanoResults'))oceanoResults.innerHTML=html;}catch(e){track('SEARCH_ERROR','SEARCH',searchQuery.value||globalSearch.value||selectedSearchCategory,{message:e.message});const html=item('Search',e.message);searchResults.innerHTML=html;if(document.getElementById('oceanoResults'))oceanoResults.innerHTML=html;}}
    ...
    function loadModule(id){setSessionState();if(id==='home')loadHome();if(id==='search'||id==='oceano')loadSearch();if(id==='mail')loadMail();if(id==='cloud')loadCloud();if(id==='tools')loadTools();if(id==='forum')loadForum();if(id==='mining')loadMining();if(id==='nodes'||id==='nas')loadNodes();if(id==='publisher')loadPublisher();setTimeout(()=>{window.veloraApplyLanguage&&window.veloraApplyLanguage();setSessionState()},0)}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;-webkit-tap-highlight-color:transparent}body{margin:0;background:radial-gradient(circle at 20% 0,#214d67,transparent 35%),linear-gradient(180deg,#06131f,#04101a 70%);color:var(--ink);font-family:ui-sans-serif,system-ui,-apple-system,Segoe UI,sans-serif}
    ...
    .row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.stack{display:grid;gap:10px}.muted{color:var(--muted)}.ok{color:var(--green)}.bad{color:var(--red)}.list{display:grid;gap:10px}.item{border:1px solid rgba(255,255,255,.1);border-radius:16px;padding:12px;background:rgba(0,0,0,.13)}.item small{color:var(--muted)}.category-bar{display:flex;gap:8px;overflow:auto;padding:4px 0 8px}.category-bar button{width:auto;white-space:nowrap;border-radius:999px;padding:9px 12px}.category-bar button.active{background:linear-gradient(135deg,var(--gold),#f6df95);color:#07131e;border-color:transparent}.category-bar span{opacity:.72;margin-left:4px}.mobile-result{display:grid;gap:10px}.mobile-result footer{display:flex;align-items:center;justify-content:space-between;gap:10px}.mobile-result em{font-style:normal;color:var(--gold);font-size:12px;text-transform:uppercase}
    ...
    body{background:radial-gradient(circle at 18% 0,rgba(103,217,255,.2),transparent 34%),radial-gradient(circle at 86% 4%,rgba(232,196,105,.14),transparent 26%),linear-gradient(180deg,#050b13,#06131f 72%)}header{padding-top:calc(12px + env(safe-area-inset-top));background:rgba(3,12,22,.82)}main{padding-bottom:calc(100px + env(safe-area-inset-bottom))}.hero,.panel{border-color:rgba(150,202,255,.16);border-radius:22px;background:linear-gradient(155deg,rgba(13,33,51,.94),rgba(5,17,29,.9));box-shadow:0 18px 58px rgba(0,0,0,.24)}.hero{padding:18px}.logo{box-shadow:0 0 24px rgba(232,196,105,.18)}input,textarea,button,select{border-color:rgba(150,202,255,.18)}button{background:linear-gradient(135deg,#183653,#0b2034)}nav.mobile{bottom:calc(10px + env(safe-area-inset-bottom));background:rgba(3,12,22,.94);box-shadow:0 18px 55px rgba(0,0,0,.38)}.card,.item{border-color:rgba(255,255,255,.1);background:rgba(255,255,255,.045)}
...
<body>
  ...
  <main>
    <section class="hero">
      <div class="kicker">Velora</div>
      <h1>Il portale Velora pronto sul telefono</h1>
    ...
    <section data-page="search" id="search">
      <div class="panel stack">
        ...
        <div id="mobileCategoryBar" class="category-bar"></div>
  ...
  <script>
    ...
    let selectedMobileCategory = '';
    const tokenKey = 'velora.mobile.token';
    const refreshKey = 'velora.mobile.refresh';
    ...
    function setMobileDynamicText(id,text){ const el=document.getElementById(id); if(!el)return; delete el.dataset.vlSrc; el.textContent=text; }
    ...
    async function loadMobileCategories(){ try{ const data=await api('/api/v1/search/categories'); const cats=(data.categories||[]).filter(c=>c.zone_count>0).slice(0,18); mobileCategoryBar.innerHTML='<button class="'+(!selectedMobileCategory?'active':'')+'" onclick="setMobileCategory(\\'\\')">Tutte</button>'+cats.map(c=>'<button class="'+(selectedMobileCategory===c.code?'active':'')+'" onclick="setMobileCategory(\\''+esc(c.code)+'\\')">'+esc(c.code)+' <span>'+esc(c.zone_count)+'</span></button>').join(''); }catch{ mobileCategoryBar.innerHTML=''; } }
    function setMobileCategory(category){ selectedMobileCategory=category; runMobileSearch(); }
    async function runMobileSearch(){ try{ await loadMobileCategories(); const raw=mobileSearch.value.trim(); const q=raw||(!selectedMobileCategory?'velora':''); const data=await api('/api/v1/search?q='+encodeURIComponent(q)+'&category='+encodeURIComponent(selectedMobileCategory)); if(!(data.results||[]).length)track('SEARCH_EMPTY','SEARCH',q||selectedMobileCategory); mobileSearchResults.innerHTML=(data.results||[]).map(r=>'<div class="item mobile-result"><b>'+esc(r.title||r.address)+'</b><small>'+esc(r.description||r.summary||'')+'</small><footer><em>'+esc(r.category||'zone')+'</em><button onclick="openMobileZone(\\''+esc(r.address||r.zone||'')+'\\')">Apri</button></footer></div>').join('')||item('Search','Nessun risultato disponibile'); }catch(e){ track('SEARCH_ERROR','SEARCH',mobileSearch.value||selectedMobileCategory,{message:e.message}); mobileSearchResults.innerHTML=item('Search',e.message); } }
    ...
    async function boot(){ const user=JSON.parse(localStorage.getItem(userKey)||'{}'); const logged=Boolean(token()); setMobileDynamicText('sessionState',logged ? 'Connesso come ' + (user.username||'utente Velora') : 'Accesso non effettuato'); const form=document.getElementById('mobileAuthForm'); if(form)form.classList.toggle('hidden',logged); const box=document.getElementById('mobileAccountBox'); if(box)box.classList.toggle('hidden',!logged); setMobileDynamicText('mobileProfileName',user.username||'utente Velora'); setMobileDynamicText('mobileProfileMail',user.mail||user.username||''); await loadHome(); }
  ...
  const body = page === "download" ? `
    ...
    <section class="public-hero">
      <div class="public-copy">
        <span class="public-kicker">VELORA</span>
...
<head>
  ...
  <style>
    ...
    .hero,.panel,.cards article{border:1px solid rgba(150,202,255,.18);background:linear-gradient(180deg,rgba(12,35,55,.82),rgba(7,20,32,.78));border-radius:28px;box-shadow:0 28px 80px rgba(0,0,0,.36)}
...
function publicZonePage(address: string, row: any, hasRuntime = false) {
  ...
  const contentAvailable = hasRuntime || Boolean(body);
  ...
  <body><main><div class="card ${found ? "" : "notfound"}"><p>Zona Velora</p><h1>${escapeHtml(found ? title : "Zona non trovata")}</h1><p>${escapeHtml(found ? description : "Questa zona non e ancora pubblicata o indicizzata. Torna al portale e cerca un risultato disponibile.")}</p><div class="meta"><span class="pill">Indirizzo: ${escapeHtml(address)}</span><span class="pill">Stato: ${escapeHtml(String(status))}</span><span class="pill">${contentAvailable ? "Contenuto disponibile" : "Contenuto non disponibile"}</span><span class="pill">Release: ${escapeHtml(String(row?.version ?? "non disponibile"))}</span></div>${!contentAvailable && found ? `<div class="auth">Il contenuto della zona non e disponibile ora. Riprova piu tardi o torna al portale per aprire un'altra zona.</div>` : ""}${body ? `<div class="body">${escapeHtml(body.slice(0, 12000))}</div>` : ""}<button data-velora-auth>Accedi con Velora</button><div class="auth" data-velora-auth-state>Accesso Velora non ancora collegato</div><p><a href="/portal">Apri portale Velora</a></p></div></main><script>
(() => {
